...
/**
 * The Class ListManagerDAOSQL.
 */
public class ListManagerDAOSQL implements Dao<ListManager> {
	...
	/**
	 * Instantiates a new list manager DAOSQL.
	 */
	public ListManagerDAOSQL() {
	 ...
	 */
	public ListManager load(ArrayList<String> infos) {
		String sql = "SELECT * FROM `person` WHERE role='manager';";
		ResultSet rs = this.queryHandler.executeQuery(sql);
		ListManager list = new ListManager();
		try {
			while (rs.next()) {
				Person p = new Person(rs.getString("name"), rs.getString("email"), new Manager());
				list.addManager(p);
			}
